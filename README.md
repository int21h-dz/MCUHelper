...
- **массив / копии**: ряд, сетка, **гексагональная решётка** (упаковка как на ТВС, до 127 позиций при R=6), отрезок, кольцо, периметр △/○₆, зеркало. На превью инструмент **Исключить** — клик по копии убирает её из вставки (исходник остаётся; «призрак» можно вернуть). При развёртке сдвига EQU исходника сохраняются (`LG2` → `LG2+3`). Кольцо и зеркало — `TRANSF R/M`, если MCU позволяет прототип; иначе — отдельные тела того же типа. Превью исходник ярче, копии тусклее; вставка одним Undo;
- превью: **три сечения** через центр черновика (XY / XZ / YZ), заливка черновика и контуры ближайших тел (не вся сцена);
- координаты под курсором; инструмент **Линейка** (длина между двумя точками, snap к вершинам/серединам/центрам/рёбрам и к H/V от первой точки);
- **палитра примитивов** (SPH, RCZ, RCC, RPP, HEX*) над сечениями: выбрать тип → клик на холсте; инструмент **Правка** — ручки центра / размера / поворота / углов RPP; параметры формы обновляются в realtime; при перетаскивании — тот же snap (в т.ч. перпендикуляр к ребру от якоря ручки);
