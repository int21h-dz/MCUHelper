...
import { evaluateExpression, parseNumbers } from "./expression";
...
export function formatBodyVolumeCm3(volume: number): string {
  if (!Number.isFinite(volume) || volume <= 0) return "—";
  if (volume >= 0.01 && volume < 1e9) return `${volume.toPrecision(4)} см³`;
 ...
 */
export function computeBodyVolumeCm3(
  body: BodyNode,
  ...
  if (t === "TRANSF") {
    if (!body.protoName || !siblings?.length) return null;
    const proto = siblings.find(
      (b) =>
        b.name.toUpperCase() === body.protoName!.toUpperCase() &&
        b.scope === body.scope &&
  ...
  switch (t) {
    case "RPP":
      ...
      return (
        Math.abs(nums[1] - nums[0]) * Math.abs(nums[3] - nums[2]) * Math.abs(nums[5] - nums[4])
      );
    case "RCZ":
...
export function computeBodyVolumeCm3FromAst(body: BodyNode, ast: DocumentAst): number | null {
