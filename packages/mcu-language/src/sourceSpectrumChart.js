...
exports.renderSourceSpectrumSvg = renderSourceSpectrumSvg;
exports.sourceSpectrumSvgDataUri = sourceSpectrumSvgDataUri;
exports.formatSourceSpectrumHover = formatSourceSpectrumHover;
function escXml(s) {
    return s
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
...
function fmtEnergyEv(eV) {
    const mev = eV / 1e6;
    if (mev >= 1)
        return `${fmtAxis(mev)}`;
    if (mev > 0)
        return `${fmtAxis(mev * 1000)}×10³`;
    return "0";
}
function renderSourceSpectrumSvg(block) {
    ...
    const ph = H - mt - mb;
    const yBase = mt + ph;
    const xOf = (e) => ml + ((e - eMin) / eSpan) * pw;
    const yOf = (p) => mt + ph - ((p - pMin) / pSpan) * ph;
    ...
    for (let i = 0; i <= 4; i++) {
        ...
        yGrid.push(`<text x="${ml - 6}" y="${y + 4}" text-anchor="end" font-size="10" fill="#7c3aed">${escXml(fmtAxis(p))}</text>`);
    }
    const xTicks = [];
    for (let i = 0; i <= 4; i++) {
        ...
        xTicks.push(`<text x="${x}" y="${H - 22}" text-anchor="middle" font-size="10" fill="#475569">${escXml(fmtEnergyEv(e))}</text>`);
    }
...
function sourceSpectrumSvgDataUri(svg) {
    return `data:image/svg+xml;base64,${Buffer.from(svg, "utf8").toString("base64")}`;
}
function formatSourceSpectrumHover(block) {
    const n = Math.min(block.energies.length, block.probabilities.length);
    const lines = ["", "---", "### Зависимость P(E)"];
    if (block.energies.length !== block.probabilities.length) {
        lines.push("", `⚠️ Число узлов EMES (${block.energies.length}) ≠ EPRO (${block.probabilities.length}); на графике **${n}** пар.`);
    }
...
//# sourceMappingURL=sourceSpectrumChart.js.map