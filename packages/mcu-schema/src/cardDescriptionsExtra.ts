...
export const EXTRA_CARD_DESCRIPTIONS: ExtraCard[] = [
  ...
  {
    label: "SI",
    title: "Нуклиды суммарного изотопа",
    syntax: "SI list",
    // ⚠ АГЕНТАМ: омоним с нуклидом кремния `SI dens` в MATR — см. siCardVsNuclide.ts / TextMate dens-lookahead.
    description:
      "Список нуклидов, входящих в суммарный изотоп. Действует с момента объявления; без list — суммарный изотоп пуст. По умолчанию нуклиды не в сумме. При одновременном SI и SINOT перед материалом активен последний объявленный список. Печать сечений суммарного изотопа — карта SIPRN. Не путать с нуклидом кремния `SI dens` в составе MATR.",
    example: "SI FP1 FP2 AM241",
    fragment: "physical",
  },
  {
    label: "SINOT",
    title: "Нуклиды вне суммарного изотопа",
    syntax: "SINOT list",
    description:
      "Список нуклидов, которые не входят в суммарный изотоп (вето, в том числе против SIDEN). Сам по себе SINOT никого в сумму не добавляет — по умолчанию нуклиды не в сумме. В сумму попадают только через SI и/или SIDEN. При одновременном SI и SINOT перед материалом активен последний объявленный список.",
    example: "SINOT U235 U238",
    fragment: "physical",
  },
  {
    label: "SIDEN",
    title: "Порог плотности для суммарного изотопа",
    syntax: "SIDEN value",
    description:
      "Все нуклиды с ядерной плотностью меньше value входят в суммарный изотоп, кроме перечисленных в активном SINOT. Карта активна всегда; объединяется с SI по «или».",
    example: "SIDEN 1.0E-8",
    fragment: "physical",
  },
  {
    label: "CPM",
    ...
    syntax: "CPM n",
    description:
      "Начинает блок размножения материалов: n — число повторений блока, номера MATR повышаются. Закрывается CPMEND.",
    fragment: "physical",
  },
  {
    label: "CPMEND",
  ...
  },
  {
    ...
    syntax: "ICE list",
    description:
      "Список элементов, подлежащих разложению на изотопы по ICE.LIB. Действует с момента объявления; без list — разложение не производится. При одновременном ICE и ICENOT перед материалом активен последний объявленный список. Можно задавать независимо для каждого материала.",
    example: "ICE Fe U",
    fragment: "physical",
  },
  {
    ...
    syntax: "ICENOT list",
    description:
      "Список элементов, не подлежащих разложению на изотопы. Действует с момента объявления; без list — разложение не производится. `ICENOT AAAA` — исключить все элементы. При одновременном ICE и ICENOT перед материалом активен последний объявленный список.",
    example: "ICENOT O N",
    fragment: "physical",
  },
  {
