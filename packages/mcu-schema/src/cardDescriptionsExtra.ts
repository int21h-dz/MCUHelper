...
export const EXTRA_CARD_DESCRIPTIONS: ExtraCard[] = [
  // --- PIN ---
  {
    ...
    description:
      "Список нуклидов, входящих в суммарный изотоп (UserGuide §8.5). Действует с момента объявления; без list — суммарный изотоп пуст. При одновременном SI и SINOT перед материалом активен последний объявленный список. Печать сечений суммарного изотопа — карта SIPRN. Не путать с нуклидом кремния `SI dens` в составе MATR.",
  ...
  },
  {
    ...
    syntax: "SINOT list",
    description:
      "Список нуклидов, не входящих в суммарный изотоп; все остальные нуклиды материала входят в него (UserGuide §8.5). Без list — суммарный изотоп пуст. При одновременном SI и SINOT перед материалом активен последний объявленный список. SIDEN действует независимо.",
  ...
  },
  {
    ...
    syntax: "SIDEN value",
    description:
      "Все нуклиды с ядерной плотностью меньше value входят в суммарный изотоп (UserGuide §8.5). Карта активна всегда, независимо от SI/SINOT; объединяется с ними по «или».",
