import type { ZoneExpr, ZoneExprBodyRef, ZoneExprComplement, ZoneExprIntersect, ZoneExprUnion } from "./types";

const BODY_NAME = /^[A-Za-z][A-Za-z0-9]{0,5}$/;
const BODY_NUM = /^\d+$/;

/** Парсер булевых выражений зон MCU-NR: `-` > ∩ > `U`. */
export function parseZoneExpression(expression: string): ZoneExpr | null {
  ...
  if (unionParts.length === 1) return parseIntersectPart(unionParts[0]);

  const operands = unionParts.map(parseIntersectPart).filter((x): x is ZoneExpr => x !== null);
  if (operands.length === 0) return null;
  if (operands.length === 1) return operands[0];
  return { kind: "union", operands } satisfies ZoneExprUnion;
}
...
function parseIntersectPart(part: string): ZoneExpr | null {
  const tokens = tokenizeIntersect(part);
  if (tokens.length === 0) return null;
  const operands = tokens.map(parseTerm).filter((x): x is ZoneExpr => x !== null);
  if (operands.length === 0) return null;
  if (operands.length === 1) return operands[0];
  return { kind: "intersect", operands } satisfies ZoneExprIntersect;
}
...
function parseTerm(token: string): ZoneExpr | null {
  if (token.startsWith("-")) {
    const name = token.slice(1);
    if (!isBodyToken(name)) return null;
    const body: ZoneExprBodyRef = { kind: "body", name };
    return { kind: "complement", operand: body } satisfies ZoneExprComplement;
  }
  if (!isBodyToken(token) || token.toUpperCase() === "U") return null;
  return { kind: "body", name: token } satisfies ZoneExprBodyRef;
}

export function collectBodyRefs(expr: ZoneExpr): string[] {
  switch (expr.kind) {
    case "body":
      return [expr.name];
    case "complement":
...
export function evalZoneExpr(expr: ZoneExpr, isInBody: (name: string) => boolean): boolean {
  switch (expr.kind) {
    case "body":
