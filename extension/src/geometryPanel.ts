import * as path from "path";
import * as vscode from "vscode";
import { LanguageClient } from "vscode-languageclient/node";
import { isMcunrDocument } from "./contentDetect";

interface PointQueryResult {
  point: { x: number; y: number; z: number };
  zone?: {
    name: string;
    materialNum?: number;
    regNum?: number;
    objNum?: number;
    expression: string;
    color: string;
  };
  material?: { number: number; nuclides: { name: string; density: string }[] };
  objectNum?: number;
  bodyHits: string[];
}

interface GeometrySceneLike {
  primitives?: { type: string; name: string; params: number[]; bbox: unknown; color?: string; zoneHint?: string }[];
  bbox?: unknown;
}

interface MeshPreviewApi {
  MESH_PREVIEW_BODY_CAP: number;
  buildMeshPreview: (
    scene: GeometrySceneLike,
    options?: { skipDetail?: boolean; bodyCap?: number }
  ) => {
    meshes: unknown[];
    unsupported: { name: string; bodyType: string; reason: string }[];
    totalBodies: number;
    detailSkipped: boolean;
    bodyCap: number;
  };
}

const GEOMETRY_REFRESH_DEBOUNCE_MS = 800;

function loadMeshPreviewApi(): MeshPreviewApi | null {
  const candidates = [
    path.join(__dirname, "..", "vendor", "mcu-geometry", "meshPreview.js"),
    path.join(__dirname, "..", "..", "packages", "mcu-geometry", "dist", "meshPreview.js"),
  ];
  for (const candidate of candidates) {
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      return require(candidate) as MeshPreviewApi;
    } catch {
      /* try next */
    }
  }
  return null;
}

export class GeometryPanel {
  private panel: vscode.WebviewPanel | undefined;
  private documentUri: string | undefined;
  private refreshTimer: ReturnType<typeof setTimeout> | undefined;
  private geometryGeneration = 0;
  /** Предпочтительный режим при открытии / после confirm. */
  private preferredMode: "2d" | "3d" = "2d";
  private meshApi = loadMeshPreviewApi();

  constructor(
    private context: vscode.ExtensionContext,
    private client: LanguageClient
  ) {}

  async show(mode: "2d" | "3d" = "2d"): Promise<void> {
    const editor = vscode.window.activeTextEditor;
    if (!editor || !isMcunrDocument(editor.document)) {
      vscode.window.showWarningMessage("Откройте файл MCU-NR");
      return;
    }

    this.documentUri = editor.document.uri.toString();
    this.preferredMode = mode;

    const scene = await this.client.sendRequest<GeometrySceneLike | null>(
      "mcuhelper/getGeometry",
      this.documentUri
    );
    if (!scene) {
      vscode.window.showErrorMessage("Не удалось построить сечение геометрии");
      return;
    }

    if (mode === "3d") {
      const ok = await this.confirmHeavy3d(scene);
      if (!ok) {
        this.preferredMode = "2d";
      }
    }

    if (this.panel) {
      this.panel.reveal();
      this.panel.title = this.preferredMode === "3d" ? "MCU-NR: Геометрия 3D" : "MCU-NR: Сечения";
      this.postScene(scene);
    } else {
      this.panel = vscode.window.createWebviewPanel(
        "mcuhelper.geometry",
        this.preferredMode === "3d" ? "MCU-NR: Геометрия 3D" : "MCU-NR: Сечения",
        vscode.ViewColumn.Beside,
        { enableScripts: true, retainContextWhenHidden: true }
      );
      this.panel.onDidDispose(() => {
        this.panel = undefined;
        this.documentUri = undefined;
        if (this.refreshTimer) clearTimeout(this.refreshTimer);
      });
      this.panel.webview.html = this.getHtml();
      this.panel.webview.onDidReceiveMessage((msg) => this.onMessage(msg));
      this.postScene(scene);
    }
  }

  async show3d(): Promise<void> {
    return this.show("3d");
  }

  /** Обновить срез при изменении отслеживаемого файла (без ручного refresh). */
  onDocumentChanged(doc: vscode.TextDocument): void {
    if (!this.panel || !this.documentUri) return;
    if (doc.uri.toString() !== this.documentUri) return;
    if (!isMcunrDocument(doc)) return;

    if (this.refreshTimer) clearTimeout(this.refreshTimer);
    this.refreshTimer = setTimeout(() => {
      void this.pushGeometryUpdate();
    }, GEOMETRY_REFRESH_DEBOUNCE_MS);
  }

  private skipDetail3d = false;

  private async confirmHeavy3d(scene: GeometrySceneLike): Promise<boolean> {
    const n = scene.primitives?.length ?? 0;
    const cap = this.meshApi?.MESH_PREVIEW_BODY_CAP ?? 500;
    if (n <= cap) {
      this.skipDetail3d = false;
      return true;
    }
    const choice = await vscode.window.showWarningMessage(
      `В геометрии ${n} тел (лимит детализации ${cap}). Показать упрощённый 3D (без мешей) или всё равно детально?`,
      "Упрощённо",
      "Детально",
      "Отмена"
    );
    if (choice === "Отмена" || choice === undefined) return false;
    this.skipDetail3d = choice === "Упрощённо";
    return true;
  }

  private postScene(scene: GeometrySceneLike): void {
    if (!this.panel) return;
    const preview = this.buildPreview(scene);
    this.panel.webview.postMessage({
      type: "scene",
      scene,
      mode: this.preferredMode,
      meshPreview: preview,
    });
  }

  private buildPreview(scene: GeometrySceneLike) {
    if (!this.meshApi) {
      return {
        meshes: [],
        unsupported: (scene.primitives ?? []).map((p) => ({
          name: p.name,
          bodyType: String(p.type).toUpperCase(),
          reason: "не в 3D",
        })),
        totalBodies: scene.primitives?.length ?? 0,
        detailSkipped: true,
        bodyCap: 500,
      };
    }
    return this.meshApi.buildMeshPreview(scene, {
      skipDetail: this.skipDetail3d,
      bodyCap: this.meshApi.MESH_PREVIEW_BODY_CAP,
    });
  }

  private async pushGeometryUpdate(): Promise<void> {
    if (!this.panel || !this.documentUri) return;
    const generation = ++this.geometryGeneration;
    try {
      const scene = await this.client.sendRequest<GeometrySceneLike | null>(
        "mcuhelper/getGeometry",
        this.documentUri
      );
      if (generation !== this.geometryGeneration) return;
      if (scene && this.panel) {
        this.postScene(scene);
      }
    } catch {
      /* LSP может быть занят; следующее изменение повторит запрос */
    }
  }

  private async onMessage(msg: {
    type: string;
    x?: number;
    y?: number;
    z?: number;
    axis?: string;
    position?: number;
    resolution?: number;
    mode?: string;
  }): Promise<void> {
    if (!this.panel || !this.documentUri) return;

    if (msg.type === "ready") {
      const scene = await this.client.sendRequest<GeometrySceneLike | null>(
        "mcuhelper/getGeometry",
        this.documentUri
      );
      if (scene) this.postScene(scene);
      return;
    }

    if (msg.type === "setMode" && (msg.mode === "2d" || msg.mode === "3d")) {
      if (msg.mode === "3d") {
        const scene = await this.client.sendRequest<GeometrySceneLike | null>(
          "mcuhelper/getGeometry",
          this.documentUri
        );
        if (scene) {
          const ok = await this.confirmHeavy3d(scene);
          if (!ok) {
            this.panel.webview.postMessage({ type: "modeRejected", mode: "2d" });
            return;
          }
          this.preferredMode = "3d";
          this.panel.title = "MCU-NR: Геометрия 3D";
          this.postScene(scene);
          return;
        }
      }
      this.preferredMode = "2d";
      this.skipDetail3d = false;
      this.panel.title = "MCU-NR: Сечения";
      this.panel.webview.postMessage({ type: "modeChanged", mode: "2d" });
      return;
    }

    if (msg.type === "queryPoint" && msg.x !== undefined && msg.y !== undefined && msg.z !== undefined) {
      const result = await this.client.sendRequest<PointQueryResult | null>("mcuhelper/queryPoint", {
        uri: this.documentUri,
        x: msg.x,
        y: msg.y,
        z: msg.z,
      });
      this.panel.webview.postMessage({ type: "pointResult", result });
      return;
    }

    if (msg.type === "getSlice" && msg.axis && msg.position !== undefined) {
      const slice = await this.client.sendRequest<unknown>("mcuhelper/getSlice", {
        uri: this.documentUri,
        axis: msg.axis,
        position: msg.position,
        resolution: msg.resolution ?? 256,
      });
      this.panel.webview.postMessage({ type: "sliceResult", slice });
    }
  }

  private getHtml(): string {
    const cssUri = this.panel!.webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "media", "geometry", "viewer.css")
    );
    const jsUri = this.panel!.webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "media", "geometry", "viewer.js")
    );
    // CSP: только локальные скрипты (CDN/Three.js недоступны) — wireframe на canvas.
    const csp = `default-src 'none'; script-src ${this.panel!.webview.cspSource}; style-src ${this.panel!.webview.cspSource} 'unsafe-inline'`;
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  <meta http-equiv="Content-Security-Policy" content="${csp}">
  <link rel="stylesheet" href="${cssUri}">
</head>
<body>
  <div id="toolbar">
    <div class="mode-switch" role="group" aria-label="Режим">
      <button type="button" id="mode2d" class="mode-btn active" data-mode="2d">2D</button>
      <button type="button" id="mode3d" class="mode-btn" data-mode="3d">3D</button>
    </div>
    <div id="toolbar-2d" class="toolbar-group">
      <label>Плоскость:
        <select id="plane">
          <option value="sliceXY" selected>XY (нормаль Z)</option>
          <option value="sliceXZ">XZ (нормаль Y)</option>
          <option value="sliceYZ">YZ (нормаль X)</option>
        </select>
      </label>
      <label>Позиция: <span id="slicePosLabel">Z=0</span>
        <input type="range" id="slicePos" min="-20" max="20" step="0.1" value="0">
      </label>
      <label>Разрешение:
        <select id="resolution">
          <option value="64">64</option>
          <option value="128">128</option>
          <option value="256" selected>256</option>
          <option value="512">512</option>
          <option value="1024">1024</option>
        </select>
      </label>
      <label>Раскраска:
        <select id="colorBy">
          <option value="material">Материал</option>
          <option value="zone">Зона</option>
        </select>
      </label>
      <button id="resetView" title="Показать весь срез">Сброс масштаба</button>
      <label>X <input type="number" id="ptX" step="0.1" value="0"></label>
      <label>Y <input type="number" id="ptY" step="0.1" value="0"></label>
      <label>Z <input type="number" id="ptZ" step="0.1" value="50"></label>
      <button id="queryBtn">Проверить точку</button>
    </div>
    <div id="toolbar-3d" class="toolbar-group" hidden>
      <button id="resetOrbit" title="Сбросить камеру">Сброс камеры</button>
      <span id="meshStats" class="hint-inline"></span>
    </div>
  </div>
  <div id="main">
    <div id="slice-wrap"><canvas id="slice-canvas"></canvas></div>
    <div id="view3d-wrap" hidden><canvas id="view3d-canvas"></canvas></div>
    <div id="sidebar">
      <div id="unsupportedBadge" class="unsupported-badge" hidden></div>
      <div id="legend"></div>
      <div id="hint" class="hint">Колёсико — зум к курсору · Shift+ЛКМ — сдвиг</div>
      <div id="info"><span class="label">Кликните на срезе для проверки точки</span></div>
    </div>
  </div>
  <script src="${jsUri}"></script>
</body>
</html>`;
  }
}
