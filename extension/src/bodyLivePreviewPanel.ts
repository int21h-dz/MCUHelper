...
import { loadBodyGeneratorApi } from "./mcuLanguageBridge";
...
/** Живые сечения тела под курсором — тот же webview, что генератор, без формы. */
export class BodyLivePreviewPanel {
  ...
  private async refreshNow(): Promise<void> {
    ...
    if (!editor || !isMcunrDocument(editor.document)) {
      if (this.panel) this.postIdle("Откройте файл MCU-NR и поставьте курсор на строку тела.");
      return;
    }
    ...
    if (!parsed) {
      if (this.panel) {
        this.postIdle("Курсор не на теле (RCZ, HEX, SPH, …). Последнее превью сохранено.");
      }
      return;
    }
    ...
    const focusKey = `${parsed.bodyType}:${parsed.name}:${startLine}`;
